...
public class LtiServlet extends HttpServlet implements ManagedService {
  ...
  protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
    ...
    try {
      ...
      if (messageType.equals(BasicLTIConstants.LTI_MESSAGE_TYPE_CONTENTITEMSELECTIONREQUEST)) {
        ...
        String customTool = URLDecoder
                .decode(StringUtils.trimToEmpty(req.getParameter(CUSTOM_TOOL)), StandardCharsets.UTF_8.displayName());
    ...
    for (Object k : req.getParameterMap().keySet()) {
      ...
      if (key.startsWith(BasicLTIConstants.CUSTOM_PREFIX) && !CUSTOM_TOOL.equals(key) && !CUSTOM_DL_TOOL.equals(key)) {
...
//    if (messageType.equals(BasicLTIConstants.LTI_MESSAGE_TYPE_CONTENTITEMSELECTIONREQUEST)) {
    ...
    builder.queryParam(BasicLTIConstants.CONTENT_ITEM_RETURN_URL,
            req.getParameter(BasicLTIConstants.CONTENT_ITEM_RETURN_URL));
//    }

    // Add locale param from LMS
    String localeParamValue = req.getParameter(BasicLTIConstants.LAUNCH_PRESENTATION_LOCALE);
